...
A CLI tool for administering LDAP users and groups on misc-lab.org. It automates user account creation, group management, credential PDF generation, and welcome email delivery.
...
```bash
scp ldap-admin-tool misc@10.0.10.8:/tmp/
ssh misc@10.0.10.8 "sudo mv /tmp/ldap-admin-tool /usr/local/bin/ && sudo chmod +x /usr/local/bin/ldap-admin-tool"
```
...
# Change email
ldap-admin-tool user modify email --uid ftotti new@misc-lab.org
...
```bash
git clone https://github.com/misc-lab/ldap-admin-tool.git
cd ldap-admin-tool
go mod tidy
